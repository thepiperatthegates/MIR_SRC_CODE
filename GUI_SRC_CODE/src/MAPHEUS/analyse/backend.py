...
plt.rcParams.update({
    'font.size': 14,
    # match the app's LIGHT_THEME window background / accent so the plot
    # doesn't look like a disconnected white rectangle in a grey window
...
class AnalyseWindow(QMainWindow, Ui_analyse_Window, AnalyseCalculationMixin):
    def __init__(self) -> None:
        ...
        refresh_icon_path = os.path.join(self.project_root, "pics", "refresh_icon.ico")
    ...
    def find_filename_button_pressed(self):
        self.analyse_filename = QFileDialog.getOpenFileName(filter="Data files (*.bin *.csv)")[0]
        if not self.analyse_filename:
            return  # user cancelled the dialog
        self.label_file.setText(QFileInfo(self.analyse_filename).fileName())
        _, type = os.path.splitext(self.analyse_filename)

        if type.lower() == ".bin":
            #skip unpacking function because it's already unpacked as it is csv file
            if not self.unpack_bin_file():
                return  # exe failed, error already printed to the console
        elif type.lower() == ".csv":
            pass  # already a CSV, nothing to unpack
